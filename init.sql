...
CREATE OR REPLACE FUNCTION update_users_summary() RETURNS TRIGGER AS $$
BEGIN
    ...
    VALUES (
        ...
        NEW.instrumentId,
        CASE
            WHEN NEW.status = 'FILLED' AND NEW.side IN ('CASH_IN', 'BUY') THEN NEW.size
            WHEN NEW.status = 'FILLED' AND NEW.side IN ('CASH_OUT', 'SELL') THEN -NEW.size
            ELSE 0
        END -- Suma o resta según el side
    )
    ON CONFLICT (userId, instrumentId) DO UPDATE
    SET
        totalSize = users_summary.totalSize + CASE
            WHEN NEW.status = 'FILLED' AND NEW.side IN ('CASH_IN', 'BUY') THEN NEW.size
            WHEN NEW.status = 'FILLED' AND NEW.side IN ('CASH_OUT', 'SELL') THEN -NEW.size
            ELSE 0
        END;
...
-- Asignar el trigger a la tabla orders
CREATE TRIGGER trg_update_users_summary
AFTER INSERT OR UPDATE ON orders
FOR EACH ROW EXECUTE FUNCTION update_users_summary();


INSERT INTO users_summary (userId, instrumentId, totalSize)
select
    o.userid,
    o.instrumentId,
    SUM(
        CASE
            WHEN o.side IN ('CASH_IN', 'BUY') THEN o.size
            WHEN o.side IN ('CASH_OUT', 'SELL') THEN - o.size
            ELSE 0
        END
    ) AS size
FROM
